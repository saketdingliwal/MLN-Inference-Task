...
boat(ori varchar(10),des varchar(10), time float) :=
  select 'spc','tfn',2.0 from dual
  union all
  select 'tfs','gmz',1.0 from dual
  union all
  select 'gmz','vde',1.5 from dual;

flight(ori varchar(10),des varchar(10), time float) :=
  select 'mad','tfn',2.0 from dual
  union all
  select 'mad','lpa',3.0 from dual
  union all
  select 'mp','vde',1.0  from dual;

bus(ori varchar(10),des varchar(10), time float) :=
  select 'tfn','tfs',2.5 from dual
  union all
  select 'lpa','mp',2.0  from dual
  union all
  select 'vde','res',1.0 from dual;
...
new_travel(ori varchar(10), des varchar(10), time float) :=
  ASSUME
    (SELECT * FROM bus WHERE bus.ori = 'vde' UNION ALL SELECT * FROM flight) NOT IN link,
